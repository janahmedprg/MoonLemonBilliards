...
def reflect(pX,pY,vX,vY,vS,r,distanceCenters,type,intersectAng1,intesectAng2,moonBilliards):
    ...
    # Calculates velocity ang relative to horizontal (used later for Phi=velocityAng - normalAng)
    if vX==0 and vY>0:
        velocityAng = np.pi/2
    elif vX==0 and vY<0:
...
N=201 # Number of collistions
nXY=35 # Number of starting positions around half of disperser type 0
nAng = 35 # Number of launch angles between -pi/2 and pi/2 or for lemon pi/2 and 3*pi/2
distanceCenters = 0.8 # Distance between the centers
...
for pX,pY,startAng in zip(xyang[0],xyang[1],xyang[2]):
    ...
    for i in range(0,N):
        ...
        if time==0:
            ...
            break
        vSPhiColor[0].append(Phi)
        vSPhiColor[1].append(vSVPP) # Change here to plot arc or spin
        vSPhiColor[2].append(color/colorMax)
    color+=1

plt.figure(figsize=(20,20))
plt.scatter(vSPhiColor[0],vSPhiColor[1],s=1,c=vSPhiColor[2],cmap='gist_rainbow')

if moonBilliards:
    fname = 'moonVPP_R'+str(round(r,3))+'_B'+str(round(distanceCenters,3))+'_eta'+str(round(eta,3))+'_N'+str(nXY*N*nAng)
else:
    fname = 'lemonVPP_R'+str(round(r,3))+'_B'+str(round(distanceCenters,3))+'_eta'+str(round(eta,3))+'_N'+str(nXY*N*nAng)
############################## Save of Show ###################################
# plt.show()
plt.axis('off')
